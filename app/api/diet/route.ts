...
/**
 * POST /api/diet
 * Generates a diet plan based on injury and profile data
 *
 * Request body: { injury: string, profile?: ProfileData }
 * Response: DietDay[] | { error: string }
 */
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Diet request user_id=${userId} injury=${injury}`);
    ...
    const prompt = buildDietPrompt(injury, profileData);

    // Get AI response
    const rawResponseText = await generateAIText(prompt);

    // Parse and normalize the response
    const dietPlan: DietDay[] = parseDietResponse(rawResponseText);

    return NextResponse.json(dietPlan);
  } catch (error) {
    console.error("Error in diet endpoint:", error);
